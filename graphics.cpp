...
mouse window::mus;
int window::move_amt = 4;
rgbcolor window::border_clr = rgbcolor{0xFF, 0x00, 0x00};
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         move_selected_object (-1 * move_amt, 0);
         break;
      case 'J': case 'j':
         move_selected_object (0, -1 * move_amt);
         break;
      case 'K': case 'k':
         move_selected_object (0, 1 * move_amt);
         break;
...
void window::special (int key, int x, int y) {
   ...
   window::mus.set (x, y);
   switch (key) {
      case GLUT_KEY_LEFT: move_selected_object
         (-1 * move_amt, 0); break;
      case GLUT_KEY_DOWN: move_selected_object
         (0, -1 * move_amt); break;
      case GLUT_KEY_UP: move_selected_object
         (0, 1 * move_amt); break;
      case GLUT_KEY_RIGHT: move_selected_object
         (1 * move_amt, 0); break;
...
void window::draw_num(object obj, size_t obj_num) {
   int width = glutBitmapWidth(GLUT_BITMAP_HELVETICA_18, 'c');
